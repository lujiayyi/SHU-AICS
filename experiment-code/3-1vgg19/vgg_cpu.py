...
def test_conv_and_pool_layer():
    print('Testing convolutional and pooling layers...')
    ...
    print('Pool layer output shape:', pool_output.shape)
    print('Layer test completed successfully!')
...
class VGG19(object):
    ...
    def load_model(self):
        ...
        for idx in range(43):
            ...
            elif 'fc' in layer_name and idx >= 37:
                # fc layers start from index 37, but params are at idx-1 in the file
                weight, bias = params['layers'][0][idx-1][0][0][0][0]
                # matconvnet: fc weights dim [height, width, in_channel, out_channel] for fc6
                # ours: fc weights dim [in_channel, out_channel]
                # Reshape: flatten spatial and channel dimensions, keep output dimension
                original_shape = weight.shape
                if len(original_shape) == 4:  # fc6: (7, 7, 512, 4096)
                    weight = weight.reshape(original_shape[0] * original_shape[1] * original_shape[2], original_shape[3])
                else:  # fc7, fc8: (1, 1, 4096, 4096) or (1, 1, 4096, 1000)
                    weight = weight.reshape(original_shape[2], original_shape[3])
